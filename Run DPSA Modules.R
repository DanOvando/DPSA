...
Assessment <- 'Galapagos Red Spiny Lobster'
...
Assessments <- c('CatchMSY','CatchCurve','DBSRA')
...
{
  FullLengthData <- read.csv(paste(Assessment,'/',Files[grepl('_LengthData',Files)],sep=''), stringsAsFactors = F)
}
if ( any(grepl('_CatchData',Files)) )
{
  FullCatchData <- read.csv(paste(Assessment,'/',Files[grepl('_CatchData',Files)],sep=''), stringsAsFactors = F)
}
if ( any(grepl('_EffortData',Files)) )
{
  FullEffortData <- read.csv(paste(Assessment,'/',Files[grepl('_EffortData',Files)],sep=''), stringsAsFactors = F)
}
if ( any(grepl('_CPUEData',Files)) )
{
  FullCPUEData <- read.csv(paste(Assessment,'/',Files[grepl('_CPUEData',Files)],sep=''), stringsAsFactors = F)
}
if ( any(grepl('_DensityData',Files)) )
{
  FullDensityData <- read.csv(paste(Assessment,'/',Files[grepl('_DensityData',Files)],sep=''), stringsAsFactors = F)
}
if ( any(grepl('_MapData',Files)) )
{
  FullLocations <- read.csv(paste(Assessment,'/',Files[grepl('_MapData',Files)],sep=''), stringsAsFactors = F)
}
...
#       iGFD<- GFD[WhereSite & GFD$CommName==Fishes[f],]
      ...
      if (exists('FullLengthData'))
      {
        LengthData <- filter(FullLengthData, Species == Fishes[f])
      }
      if (exists('FullCatchData'))
      {
        CatchData <- filter(FullCatchData, Species == Fishes[f])
      }
      if (exists('FullEffortData'))
      {
        EffortData <- filter(FullEffortData, Species == Fishes[f])
      }
      if (exists('FullCPUEData'))
      {
        CPUEData <- filter(FullCPUEData, Species == Fishes[f])
      }
      if (exists('FullDensityData'))
      {
        DensityData <- filter(FullDensityData, Species == Fishes[f])
      }
      ...
      SpeciesLifeHistory<- LifeHistory[LifeHistory$CommName == Species[f],colnames(LifeHistory) %in% LifeData]

      sTaxa<- LifeHistory$Taxa[LifeHistory$CommName == Species[f]]

      HasLifeHistory<- SpeciesLifeHistory[which(is.na(SpeciesLifeHistory)==F)]

      HasLife<- colnames(HasLifeHistory)

      Fish$CommName <- Species[f]
      ...
      Theme<- theme(legend.position='top',plot.background=element_rect(color=NA),
                    ...
                    ,text=element_text(size=10,family=Font,color=FontColor),
...
#       MapCCFRP(ReformData)
      ...
      for (a in 1:length(Assessments)) #Loop over possible assessments, store in Assessment results. Many assessments have more detailed outputs than can also be accessed
      {
        ...
        if (Assessments[a]=='CatchMSY')
        {
          ...
          Temp2<- CatchMSY(Data = CatchData, n = 1000, ProcessError = 0.05, phi = 0.188,
                           Smooth = 0, Display = T, CatchJitters = 1, CatchError = 0, CatchBias = 0,
                           InterpCatch = 1,StartYear = NA, startbio = c(0.75,0.99),
                           MidYear = NA, finalbio = c(0.25,0.6))
        ...
        if (Assessments[a]=='DBSRA') #Run DBSRA Assessment
        {
          ...
          Temp2<- DBSRA(CatchDat = CatchData, DCAC.start.yr = min(CatchData$Year), DCAC.end.yr = max(CatchData$Year), 
                        delta.yr = max(CatchData$Year), DBSRA.OFL.yr = max(CatchData$Year), FMSYtoMRatio = 0.8, 
                        SD.FMSYtoMratio = 0.05, Delta = 0.6, SD.Delta = 0.1, DeltaLowerBound = 0.4, 
                        DeltaUpperBound = 0.9, BMSYtoB0ratio = 0.4, SD.BMSYtoB0ratio = 0.1, 
                        BMSYtoB0LowerBound = 0.2, BMSYtoB0UpperBound = 0.5, InterpCatch = 1, 
                        Niter = NumIterations)
